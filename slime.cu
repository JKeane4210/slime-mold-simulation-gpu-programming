...
void motor_stage_cpu(SlimeParticle* p) {
    ...
    if (!occupied[(int)round(n_y)][(int)round(n_x)]) {
        ...
        p->x = n_x;
        p->y = n_y;
        if ((int)round(p->y) < ENV_HEIGHT && (int)round(p->y) >= 0 && (int)round(p->x) < ENV_WIDTH && (int)round(p->x) >= 0)
            env[(int)round(p->y)][(int)round(p->x)] += depT; // deposit trail in new location
    } else {
...
void decay_chemoattractant() {
    for (int i = 0; i < ENV_HEIGHT; ++i) {
        for (int j = 0; j < ENV_WIDTH; ++j) {
            env[i][j] = max(env[i][j] - decayT, 0.0);
        }
    }
}
...
void gray_scale_image_to_file(const char *cpu_output_file) {
    ...
    for (int i = 0; i < ENV_HEIGHT; ++i)
    {
        for (int j = 0; j < ENV_WIDTH; ++j)
        {
            fputc((int)(min(255.0, env[i][j])), output_file_handle);
        }
    }
